...
def votingPg(root,frame1,client_socket):
    ...
    Radiobutton(frame1, text = "Red Party", variable = vote, value = "bjp", indicator = 0, height = 4, width=15, command = lambda: voteCast(root,frame1,"bjp",client_socket)).grid(row = 2,column = 1)
    ...
    Radiobutton(frame1, text = "Blue Party", variable = vote, value = "cong", indicator = 0, height = 4, width=15, command = lambda: voteCast(root,frame1,"cong",client_socket)).grid(row = 3,column = 1)
    congLogo = ImageTk.PhotoImage((Image.open("img/cong.png")).resize((35,48),Image.ANTIALIAS))
