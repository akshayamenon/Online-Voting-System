...
def AdminHome(root,frame1,frame3):
    ...
    Button(frame3, text="Admin",bg='White', command = lambda: AdminHome(root, frame1, frame3)).grid(row = 1, column = 0)
    ...
    #Admin Login
    runServer = Button(frame1, text="Run Server",bg='red', width=15, command = lambda: sb_p.call('start python Server.py', shell=True))

    #Voter Login
    registerVoter = Button(frame1, text="Register Voter",bg='green', width=15, command = lambda: regV.Register(root, frame1))

    #Show Votes
    showVotes = Button(frame1, text="Show Votes", width=15,bg='gray', command = lambda: adFunc.showVotes(root, frame1))
...
def log_admin(root,frame1,admin_ID,password):
    ...
    if(admin_ID=="CN" and password=="cn"):
